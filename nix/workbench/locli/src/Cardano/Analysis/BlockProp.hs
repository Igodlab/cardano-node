...
mbeBlockHash = mapMbe bfeBlock boeBlock eBlock
    ...
    where go Nothing  acc = acc
          go (Just h) acc =
            case partitionMbes $ mapMaybe (Map.lookup h) machBlockMaps of
              ([], _, _) -> error $ "No forger for hash " <> show h
  ...
  LogObject{loAt, loHost, loBody=LOBlockFetchClientCompletedFetch{loBlock}} ->
    ...
    in
      ...
      <&> (\v ->
           ...
           mbe0)
      & fromMaybe (fail loBlock "LOBlockFetchClientCompletedFetch leads fetching")
      & doInsert loBlock
  LogObject{loAt, loHost, loBody=LOBlockAddedToCurrentChain{loBlock,loChainLengthDelta}} ->
    let mbe0 = Map.lookup loBlock bMap
               & fromMaybe (fail loBlock "LOBlockAddedToCurrentChain leads")
    in if isJust (mbeAdopted mbe0) then bMap else
      ...
      <&> (\v ->
          ...
          mbe0)
      & fromMaybe (fail loBlock "LOBlockAddedToCurrentChain leads acquirement")
      & doInsert loBlock
  LogObject{loAt, loHost, loBody=LOChainSyncServerSendHeader{loBlock}} ->
    let mbe0 = Map.lookup loBlock bMap
               & fromMaybe (fail loBlock "LOChainSyncServerSendHeader leads")
    in if isJust (mbeAnnounced mbe0) then bMap else
      ...
      <&> (\v ->
          ...
          mbe0)
      & fromMaybe (fail loBlock "LOChainSyncServerSendHeader leads adoption")
      & doInsert loBlock
  LogObject{loAt, loHost, loBody=LOBlockFetchServerSending{loBlock}} ->
    let mbe0 = Map.lookup loBlock bMap
               & fromMaybe (fail loBlock "LOBlockFetchServerSending leads")
    in if isJust (mbeSending mbe0) then bMap else
      ...
      <&> (\v ->
           ...
           mbe0)
      & fromMaybe (fail loBlock "LOBlockFetchServerSending leads announcement")
      & doInsert loBlock
  _ -> bMap
 where
